"""
patch_onboard.py — Replace the /onboard redirect stub with a full inline onboarding modal.
"""
import re
...
# ── Replace old handleAuthReturn + routeToStep + showOnboardingModal stub ──────
old_pattern = r'    // Handle return from OAuth.*?// For now: redirect to /onboard page.*?window\.location\.href = \'/onboard\?' \+ params\.toString\(\);\n    \}'
new_js = r'''    // ══ Handle return from OAuth (backend sets HttpOnly cookie, redirects with ?step=) ══
    (function handleAuthReturn() {
        const params = new URLSearchParams(window.location.search);
        const step = params.get('step');
        if (!step) return;

        // Restore role if pending
        ...
        // Route to correct onboarding step (all inline — no /onboard redirect)
    ...
    let _obAnswers = {};   // { question_id: chosen_answer_text }
    let _obQuestions = []; // questions from /api/onboarding/test/start
    let _obAnswerKey = {}; // server-returned answer key after grading
    ...
    async function _renderObStep(step) {
        ...
        if (step === 'role') {
            el.innerHTML = `
                ...
                <p class="sub">Select your role to begin your Electronic Notary Public (ENP) certification.</p>
            ...
            el.innerHTML = `
                ...
                <p class="sub">Your information is required for ENP certification under A.M. No. 24-10-14-SC.</p>
                ...
                <div class="onboard-field"><label>Notarial Jurisdiction (if attorney)</label><input type="text" id="ob-jurisdiction" placeholder="e.g. Albay"></div>
            ...
            el.innerHTML = `
                ...
                <p class="sub">A quick liveness check is required under BSP Circular 944 (eKYC) to complete your certification.</p>
                <div style="border:1px solid rgba(255,255,255,0.1);border-radius:0.75rem;padding:1.5rem;text-align:center;margin-bottom:1.25rem;background:rgba(255,255,255,0.03);">
                    ...
                    <p style="color:var(--muted);font-size:0.88rem;margin:0 0 1rem;">
                        Your camera will be used to verify your identity. Please ensure good lighting and look directly at the camera.
                    </p>
                    <button class="onboard-btn" style="max-width:220px;margin:0 auto;display:block;" onclick="_startLiveness()">Start Liveness Check</button>
                </div>
                <div id="ob-err" class="ob-error"></div>`;
    ...
    async function _submitRole() {
        ...
        btn.disabled = true;
        btn.innerHTML = '<span class="ob-spinner"></span>Saving…';
        try {
    ...
    async function _loadTest() {
        try {
            ...
            document.getElementById('ob-content').innerHTML = `<h3>Test</h3><div class="ob-error">${e.message}</div>`;
        }
    }
    ...
    function _renderTestUI() {
        ...
        const qs = _obQuestions.map((q, i) => `
            <div class="ob-question" id="q-${q.id}">
                <p>${i+1}. ${q.question}</p>
                ${q.choices.map(c => `
                    <div class="ob-choice" onclick="_pickAnswer('${q.id}','${c.replace(/'/g,"\\'")}',this)">${c}</div>
                `).join('')}
            </div>`).join('');
        el.innerHTML = `
    ...
    function _pickAnswer(qid, answer, el) {
        _obAnswers[qid] = answer;
    ...
    async function _submitTest() {
        const answered = Object.keys(_obAnswers).length;
        if (answered < _obQuestions.length) {
            document.getElementById('ob-err').textContent = `Please answer all questions (${answered}/${_obQuestions.length} answered).`;
            return;
        }
    ...
    function _renderTestResult(data) {
        ...
        el.innerHTML = `
            ...
            <div class="ob-result-banner ${passed?'pass':'fail'}">
                ${passed ? '🎉 Passed!' : '❌ Not Passed'} — ${score}/${total} correct (${Math.round(score/total*100)}%)
            </div>
            <p class="sub">${passed
                ? 'Congratulations! Proceed to the final identity verification step.'
                : 'You need 80% to pass. Review the Philippine ENP rules and try again.'
            }</p>
            ${passed
    ...
    async function _startLiveness() {
        const btn = event.target;
        btn.disabled = true; btn.innerHTML = '<span class="ob-spinner"></span>Starting camera…';
        try {
            // Request camera permission
            const stream = await navigator.mediaDevices.getUserMedia({ video: true });
            stream.getTracks().forEach(t => t.stop()); // just permission check for now

            // POST liveness result to backend
...
match = re.search(
    r'    // Handle return from OAuth.*?window\.location\.href = \'/onboard\?' \+ params\.toString\(\);\n    \}',
    content, re.DOTALL
)
if match:
    content = content[:match.start()] + new_js + content[match.end():]
    print(f"Replaced block ({match.end()-match.start()} chars → {len(new_js)} chars)")
else:
    print("ERROR: old block not found")

# ── Also fix showCertifiedModal stub ──────────────────────────────────────────
old_cert = "    function showCertifiedModal(token) {\n        alert('dYZ% You are fully certified! Check your email for your certificate.');\n    }"
if old_cert in content:
    content = content.replace(old_cert, "")
    print("Removed showCertifiedModal stub")
else:
    # Try partial
    idx = content.find('function showCertifiedModal')
    print(f"showCertifiedModal at {idx}")
